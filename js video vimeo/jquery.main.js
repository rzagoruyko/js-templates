...
function initVideo() {
  jQuery('.video-box').each(function () {
    ...
    var vimeoIframe = videoBox.find('.video');
    var vimeoVideo;

    function readyVimeo(){
      if((typeof Vimeo !== "undefined") && Vimeo && Vimeo.Player){
        vimeoVideo = new Vimeo.Player(vimeoIframe);

        var srcProp = jQuery(vimeoVideo.element).prop('src');

        jQuery(vimeoVideo.element).prop('src', srcProp + '?portrait=0' + '&title=0' + '&byline=0');
      }else{
        setTimeout(readyVimeo, 100);
      }
    }
...
function initCustomVideo() {
  jQuery('.custom-video-box').each(function () {
    ...
    function readyVimeo(){
      var srcProp = vimeoIframe.data('src');

      vimeoIframe.attr('src', srcProp + '?portrait=0' + '&title=0' + '&byline=0' + '&autoplay=1');
